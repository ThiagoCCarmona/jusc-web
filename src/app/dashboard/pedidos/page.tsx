...
export default function PedidosPage() {
  ...
  const [campanhaEditandoId, setCampanhaEditandoId] = useState<string | null>(null);
  const [tituloCamp, setTituloCamp] = useState("");
  const [descCamp, setDescCamp] = useState("");
  const [precoCamp, setPrecoCamp] = useState("");
  const [modelosCamp, setModelosCamp] = useState<ModeloPrecoItem[]>([
    { nome: "Tradicional" },
    { nome: "Baby Look" },
  ]);
  const [novoModeloNome, setNovoModeloNome] = useState("");
  const [novoModeloPreco, setNovoModeloPreco] = useState("");
  const [tamanhosCamp, setTamanhosCamp] = useState<string[]>(["PP", "P", "M", "G", "GG", "XGG"]);
  const [permiteNomeCamp, setPermiteNomeCamp] = useState(true);
  const [permiteNumeroCamp, setPermiteNumeroCamp] = useState(true);
  const [dataFimCamp, setDataFimCamp] = useState("");
  const [fotosCamp, setFotosCamp] = useState<FotoComLabel[]>([]);
  const [uploadingFotoCamp, setUploadingFotoCamp] = useState(false);
  ...
  // Upload de Foto para Campanha com suporte a Label
  async function handleUploadFoto(file: File) {
    const formData = new FormData();
    formData.append("file", file);
    setUploadingFotoCamp(true);
    try {
      const res = await fetch("/api/upload", { method: "POST", body: formData });
      const data = await res.json();
      if (res.ok && data.url) {
        setFotosCamp((prev) => [...prev, { url: data.url, label: "" }]);
      } else {
        setMensagemErro(data.error || "Erro no upload da foto.");
      }
    } catch {
      setMensagemErro("Erro de conexão no envio da foto.");
    } finally {
      setUploadingFotoCamp(false);
    }
  }
  ...
  function abrirCriarCampanha() {
    ...
    setNovoModeloNome("");
    setNovoModeloPreco("");
    setTamanhosCamp(["PP", "P", "M", "G", "GG", "XGG"]);
    setPermiteNomeCamp(true);
    setPermiteNumeroCamp(true);
    setDataFimCamp("");
    setFotosCamp([]);
  ...
  function abrirEditarCampanha(camp: CampanhaItem) {
    setCampanhaEditandoId(camp.id);
    setTituloCamp(camp.titulo);
    setDescCamp(camp.descricao || "");
    setPrecoCamp(camp.precoUnitario.toString());
    const modelosNorm = normalizarModelos(camp.modelos, camp.precoUnitario);
    setModelosCamp(modelosNorm);
    setNovoModeloNome("");
    setNovoModeloPreco("");
    setTamanhosCamp(Array.isArray(camp.tamanhosDisponiveis) ? camp.tamanhosDisponiveis : ["P", "M", "G"]);
    setPermiteNomeCamp(camp.permiteNome);
    setPermiteNumeroCamp(camp.permiteNumero);

    const dataFimStr = camp.dataFim ? new Date(camp.dataFim).toISOString() : "";
    setDataFimCamp(dataFimStr);

    const fotosFormatadas: FotoComLabel[] = (camp.fotos || []).map((f) =>
      typeof f === "string" ? { url: f, label: "" } : f
    );
    setFotosCamp(fotosFormatadas);
    setModalCampanhaAberto(true);
  }

  async function handleSalvarCampanha(e: React.FormEvent) {
    e.preventDefault();
    if (!tituloCamp || !precoCamp || !dataFimCamp) {
      setMensagemErro("Preencha título, valor unitário base e data de encerramento.");
      return;
    }
    if (fotosCamp.length < 2) {
      setMensagemErro("Adicione no mínimo 2 fotos da camiseta.");
      return;
    }
    if (modelosCamp.length === 0) {
      setMensagemErro("Adicione pelo menos um modelo para a camiseta.");
      return;
    }

    setSalvandoCampanha(true);
    try {
      const precoBase = parseFloat(precoCamp);
      const payload = {
        titulo: tituloCamp,
        descricao: descCamp,
        precoUnitario: precoBase,
        modelos: modelosCamp.map((m) => ({
          nome: m.nome.trim(),
          preco: m.preco !== undefined && !isNaN(m.preco) && m.preco > 0 ? m.preco : precoBase,
        })),
        tamanhosDisponiveis: tamanhosCamp,
        permiteNome: permiteNomeCamp,
        permiteNumero: permiteNumeroCamp,
        dataFim: dataFimCamp,
        fotos: fotosCamp,
      };

      const url = campanhaEditandoId ? `/api/campanhas/${campanhaEditandoId}` : "/api/campanhas";
      const method = campanhaEditandoId ? "PUT" : "POST";
      ...
      if (res.ok) {
        setMensagemSucesso(
          campanhaEditandoId
            ...
            : "Nova campanha de camisetas iniciada com sucesso!"
        );
        setTimeout(() => setMensagemSucesso(""), 4000);
        setModalCampanhaAberto(false);
        carregarDados();
      } else {
        setMensagemErro(data.error || "Falha ao salvar campanha.");
      }
    } catch {
      setMensagemErro("Erro de conexão ao salvar campanha.");
    } finally {
          ...
          <div className="bg-white dark:bg-[#15171e] rounded-3xl p-6 max-w-2xl w-full border border-neutral-200 dark:border-neutral-800 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-neutral-100 dark:border-neutral-800 pb-3">
              <h2 className="text-base font-black text-neutral-900 dark:text-white flex items-center gap-2">
                ...
                {campanhaEditandoId ? "Editar Campanha de Camisetas" : "Iniciar Nova Campanha de Camisetas"}
              </h2>
              <button
                type="button"
            ...
            <form onSubmit={handleSalvarCampanha} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    value={tituloCamp}
                    onChange={(e) => setTituloCamp(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 rounded-xl bg-neutral-50 dark:bg-[#1c202a] border border-neutral-300 dark:border-neutral-700 text-xs font-bold"
                  />
                  <span className="text-[10px] text-neutral-400">Preço padrão para modelos sem valor específico</span>
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-neutral-700 dark:text-neutral-300 mb-1">
                    Descrição e Instruções de Retirada
                  </label>
                  <textarea
                    rows={2}
                    value={descCamp}
                    onChange={(e) => setDescCamp(e.target.value)}
                    placeholder="Ex.: Malha 100% algodão. Entrega nos encontros de domingo..."
                ...
                {/* Modelos e Valores Personalizados por Modelo */}
                <div className="sm:col-span-2 space-y-2.5 p-3.5 rounded-2xl bg-neutral-50 dark:bg-[#1c202a] border border-neutral-200 dark:border-neutral-800">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-neutral-500">
                        Defina modelos da camiseta e opcionalmente atribua um valor específico para cada um (ex: Tradicional R$ 40, Moletom R$ 80).
                      </p>
                    </div>
                  </div>
                      ...
                      >
                        <div className="flex-1">
                          <input
                            ...
                            value={mod.nome}
                            onChange={(e) => {
                              const novos = [...modelosCamp];
                              novos[idx] = { ...novos[idx], nome: e.target.value };
                              setModelosCamp(novos);
                            }}
                        ...
                        <div className="w-32">
                          <div className="relative">
                            <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] text-neutral-400 font-bold">R$</span>
                            <input
                              type="number"
                              step="0.01"
                              min="0"
                              placeholder={precoCamp || "45.00"}
                              ...
                              className="w-full pl-7 pr-2 py-1.5 rounded-lg bg-neutral-50 dark:bg-[#1c202a] border border-neutral-300 dark:border-neutral-700 text-xs font-bold"
                            />
                          </div>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            if (modelosCamp.length <= 1) {
                              setMensagemErro("A campanha deve ter ao menos um modelo.");
                              return;
                            }
                            setModelosCamp(modelosCamp.filter((_, i) => i !== idx));
                          }}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <input
                      type="text"
                      placeholder="Novo modelo (ex: Regata, Baby Look, Moletom)"
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          ...
                          if (novoModeloNome.trim()) {
                            const p = novoModeloPreco ? parseFloat(novoModeloPreco) : (precoCamp ? parseFloat(precoCamp) : undefined);
                            setModelosCamp([...modelosCamp, { nome: novoModeloNome.trim(), preco: p }]);
                            setNovoModeloNome("");
                            setNovoModeloPreco("");
                          }
                        }
                      }}
                      className="flex-1 px-3 py-1.5 rounded-xl bg-white dark:bg-[#15171e] border border-neutral-300 dark:border-neutral-700 text-xs"
                    />
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!novoModeloNome.trim()) {
                          setMensagemErro("Informe o nome do modelo.");
                          return;
                        }
                        const p = novoModeloPreco ? parseFloat(novoModeloPreco) : (precoCamp ? parseFloat(precoCamp) : undefined);
                        setModelosCamp([...modelosCamp, { nome: novoModeloNome.trim(), preco: p }]);
                      ...
                      className="px-3 py-1.5 rounded-xl bg-neutral-900 dark:bg-white text-white dark:text-neutral-950 font-bold text-xs hover:opacity-90 flex items-center gap-1.5 flex-shrink-0"
                    >
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <InputDataBr
                      value={dataFimCamp ? dataFimCamp.slice(0, 10) : ""}
                      onChange={(br, iso) => {
                        const hora = dataFimCamp.includes("T") ? dataFimCamp.split("T")[1] : "23:59";
                        setDataFimCamp(iso ? `${iso}T${hora}` : "");
                      }}
                    ...
                    <input
                      type="time"
                      value={dataFimCamp.includes("T") ? dataFimCamp.split("T")[1].slice(0, 5) : "23:59"}
                      onChange={(e) => {
                        const dataBase = dataFimCamp.includes("T") ? dataFimCamp.split("T")[0] : new Date().toISOString().slice(0, 10);
                        setDataFimCamp(`${dataBase}T${e.target.value}`);
                      }}
                ...
                <div className="sm:col-span-2 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["12", "14", "16", "PP", "P", "M", "G", "GG", "XGG", "G1", "G2"].map((tam) => {
                      const sel = tamanhosCamp.includes(tam);
                      return (
                        <button
                          key={tam}
                          type="button"
                          onClick={() => {
                            if (sel) setTamanhosCamp(tamanhosCamp.filter((t) => t !== tam));
                            else setTamanhosCamp([...tamanhosCamp, tam]);
                          }}
                ...
                <div className="sm:col-span-2 flex items-center gap-6 p-3 rounded-xl bg-neutral-50 dark:bg-[#1c202a] border border-neutral-200 dark:border-neutral-700">
                  <label className="flex items-center gap-2 cursor-pointer font-bold">
                    <input
                      type="checkbox"
                      checked={permiteNomeCamp}
                      onChange={(e) => setPermiteNomeCamp(e.target.checked)}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer font-bold">
                    <input
                      type="checkbox"
                      checked={permiteNumeroCamp}
                      onChange={(e) => setPermiteNumeroCamp(e.target.checked)}
                      className="w-4 h-4 rounded text-[#FFC72C]"
                    />
                ...
                {/* Upload de Fotos com Legenda/Label e Capa */}
                <div className="sm:col-span-2 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <label className="block text-xs font-bold text-neutral-700 dark:text-neutral-300">
                        Fotos da Camiseta * (mínimo 2 fotos, adicione legendas aos modelos)
                      </label>
                      <p className="text-[11px] text-neutral-500">
                        Você pode etiquetar cada imagem (ex: "Frente", "Costas", "Baby Look", "Tabela de Medidas").
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        disabled={uploadingFotoCamp}
                        className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 font-bold"
                      >
                        <Upload className="w-3.5 h-3.5 text-[#FFC72C]" />
                        {uploadingFotoCamp ? "Enviando..." : "Adicionar Foto"}
                      </button>
                    </div>
                  </div>
                              ...
                              <div className="flex items-center justify-between">
                                {ehCapa ? (
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => {
                                      const reordenadas = [foto, ...fotosCamp.filter((_, i) => i !== idx)];
                                      setFotosCamp(reordenadas);
                                    }}
                                ...
                                <button
                                  type="button"
                                  onClick={() => setFotosCamp(fotosCamp.filter((_, i) => i !== idx))}
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-neutral-100 dark:border-neutral-800">
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-[#FFC72C] hover:bg-[#e5b220] text-neutral-950 font-black shadow-sm"
                >
                  {salvandoCampanha ? "Salvando..." : campanhaEditandoId ? "Salvar Alterações" : "Publicar Campanha"}
                </button>
              </div>
            </form>
          </div>
